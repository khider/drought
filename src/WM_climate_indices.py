...
def openCESM(dataset_name, bounding_box, globe):
    '''
    Open CESM data to calculate the indices
    ...
    dataset_name : str
        Name of the dataset
    bounding_box : list
        A list containing [min_lon, max_lon, min_lat, max_lat].
    globe : bool
        Whether to use the entire globe.


    Returns
    -------
    da_precip: xarray dataarray
        The Xarray Dataarray object containing the precipitation data
    da_temp: xarray dataarray
        The Xarray Dataarray object containing the temperature data
    '''
    ...
    if globe == True:
        ...
        da_precip = p_.PRECIP
        da_temp = p_.TS
    ##unit transformation m/s to mm
    time = da_precip.time.values
    t=[]
    for item in time:
        t.append(item.strftime("%m"))
    days=[]
    for item in t:
        days.append(monthrange(2019,int(item))[1])
    #Precip unit conversion
    da_precip = da_precip*1000*86400
    da_precip.attrs['units'] = 'mm'
    ### TEMPERATURE
    ## Unit conversion
    da_temp = da_temp-273.15
    ...
    time_list = []
    for item in time:
        time_list.append(item.isoformat())
    da_precip = da_precip.assign_coords(time=time_list)
    da_temp = da_temp.assign_coords(time=time_list)
...
def SPI(da_precip, distribution = 'gamma', periodicity = 'monthly',\
    ...
    if isinstance(da_precip.time.values[0],np.str_):
        pass
    else:
    ...
    #cut
    if len(str(data_start_year))<4:
        t_start  = '0'+str(data_start_year)+'-01-01T00:00:00.000000000'
    else:
        t_start  = str(data_start_year)+'-01-01T00:00:00.000000000'
    if len(str(data_end_year))<4:
        t_end  = '0'+str(data_end_year)+'-12-31T00:00:00.000000000'
    else:
        t_end  = str(data_end_year)+'-12-31T00:00:00.000000000'
...
def to_netcdfMint(ds, info, dataset_type, dir_out, dynamic_name):
    ...
    Args:
        ...
        param_list (list): list of parameters used for the runs
    ...
    if info['index'] == 'SPI':
        ...
        ds.attrs['description'] = info['periodicity']+' standardized precipitation index inferred from '\
        ...
        ds.attrs['title'] = long_name
        ds.attrs['description'] = 'Monthly potential evapotranspiration inferred from '\
        + dataset_type +'.'
        info['periodicity']='monthly'
    elif info['index'] == 'SPEI':
        long_name = 'Standardized Precipitation Evapotranspiration Index'
        ds.attrs['title'] = long_name
        ds.attrs['description'] = info['periodicity']+' standardized precipitation  evapotranspiration index inferred from '\
        + dataset_type +', using a calibration period from '+\
        str(info['calibration_start']) + ' to '+ str(info['calibration_end'])+\
            ' using a '+ info['distribution']+ ' distribution with a '+\
            str(info['timescales'])+ '-month timescale.'
    ...
    ds.attrs['dependent_vars'] = [info['index']]
    ds.attrs['tags'] = ['Drought', 'Climate', 'SPI', 'SPEI']
    if 'latitude' in ds.coords:
        ...
        ds.attrs['independent_vars'] = ['lat', 'lon', 'time']
    elif 'Y' in ds.coords:
        ...
        ds.attrs['independent_vars'] = ['X', 'Y', 'time']
...
if __name__ == "__main__":
    ...
    ## Perform checks:
    #possible datatypes
    dataset_type_list = ['CHIRPS','GLDAS','FLDAS','ECMWF','CESM']
    if dataset_type not in dataset_type_list:
        raise ValueError("Dataset type not a valid entry. Use either 'CHIRPS', 'GLDAS', 'FLDAS', 'ECMWF','CESM'")
    #possible indices
    ...
    if dataset_type == 'CHIRPS' or dataset_type=='CESM':
    ...
    elif dataset_type == 'CESM':
        da_precip, da_temp = openCESM(dataset_name, bounding_box, globe)
